...
export async function tickets_technicians_total(models) {
  //arreglo de tecnicos
  const technicians = [];
  ...
  try {
    ...
    for (let i = 0; i < usuarios.length; i++) {
      ...
      const casosTotal = await models.glpi_tickets_users.findAll({
        attributes: ["id"],
        where: {
          users_id: usuarios[i].users_id,
        },
      });
      ...
      technicians.push({
        ...
        cantidad: casosMes,
        cantidadTotal: casosTotal.length,
      });
